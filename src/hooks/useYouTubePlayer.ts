...
export function useYouTubePlayer({ videoId, origin, onProgressTick, onStateChange, autoPlayOnReady }: UseYouTubePlayerOptions) {
  ...
  useEffect(() => {
    ...
    pollRef.current = window.setInterval(() => {
      ...
      // BUGFIX: getAvailableQualityLevels() was only ever read once, in
      // onReady, before YouTube has actually determined the video's
      // renditions — it reliably returned an empty list at that point, so
      // the quality button (rendered only when the list is non-empty) never
      // showed up. Re-checking here, throttled to whenever it actually
      // changes, means it appears as soon as YouTube reports it — typically
      // within the first second or two of buffering.
      const qualities = p.getAvailableQualityLevels?.() ?? [];
      const key = qualities.join(",");
      if (key !== lastQualitiesKey) {
        lastQualitiesKey = key;
        patch({ availableQualities: qualities });
      }
