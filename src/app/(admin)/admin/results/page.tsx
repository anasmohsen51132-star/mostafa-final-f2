...
// Quiz and homework results now share an identical shape and an identical
// table layout — homework is graded exactly like quizzes are.
...
export default function ResultsPage() {
  ...
  const filtered = rows.filter((r) => {
    ...
    const passMatch = filterPass === "all" ||
      (filterPass === "passed" && r.passed) ||
      (filterPass === "failed" && !r.passed);
    return nameMatch && passMatch;
  });

  // Aggregated stats — reflect whichever tab (quiz/homework) is active,
  // since both now carry the exact same percentage/passed shape.
    ...
    <div style={{ direction: "rtl" }}>
      ...
      {!isLoading && (
        <StaggerContainer className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {[
            { icon: "📝", value: totalAttempts,  label: activeTab === "quiz" ? "محاولة اختبار كلي" : "محاولة واجب كلي", color: "#C9A84C" },
      ...
      {/* Table — identical layout for quiz and homework now */}
      {isLoading ? (
        ...
        <div className="text-center py-20 rounded-2xl"
          ...
          <p style={{ fontFamily: "Cairo,sans-serif", color: "#7A6E5A", fontSize: 14 }}>
            {searchName ? "جرب البحث بكلمات أخرى" : `ستظهر النتائج هنا عند تسليم الطلاب لـ${activeTab === "quiz" ? "لاختبارات" : "لواجبات"}`}
          </p>
        </div>
      ) : (
        <div className="rounded-2xl overflow-hidden"
          ...
          <div className="grid gap-3 px-5 py-3 text-xs font-semibold"
            ...
            <span>الكورس</span>
            <span>المحاضرة</span>
            <span>{activeTab === "quiz" ? "الاختبار" : "الواجب"}</span>
            <span>المحاولة</span>
            <span>الدرجة</span>
            <span>الحالة</span>
          </div>
          {filtered.map((r, i) => (
            ...
            >
              ...
              <span style={{ fontFamily: "Cairo,sans-serif", color: "#4A3F2A", fontSize: 12 }}>
                {r.type === "quiz" ? r.quizTitle : r.homeworkTitle}
              </span>
              <span
