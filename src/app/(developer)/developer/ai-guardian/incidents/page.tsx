"use client";
// src/app/(developer)/developer/ai-guardian/incidents/page.tsx
...
export default function IncidentsPage() {
  ...
  const incidents = [...(report?.reportJson.criticalIncidents ?? []), ...(report?.reportJson.warnings ?? [])];
  const loading = isLoading || loadingReport;

  return (
    <div style={{ direction: "rtl" }}>
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
        <h1 style={{ fontFamily: "Amiri,serif", color: "#1A1208", fontSize: 32, marginBottom: 4 }}>🧩 تحليل الحوادث</h1>
        <p style={{ fontFamily: "Cairo,sans-serif", color: "#7A6E5A", fontSize: 14 }}>
          {report ? `من آخر تقرير — ${formatGuardianDateTime(report.createdAt)} — تحليل السبب الجذري لكل حادثة` : "لا يوجد تقرير بعد"}
        </p>
      </motion.div>

      {loading && <div className="space-y-3">{[...Array(3)].map((_, i) => <div key={i} className="skeleton rounded-2xl h-24" />)}</div>}
      ...
      {!loading && latestId && incidents.length === 0 && (
        <div className="rounded-2xl p-10 text-center" style={{ background: "#fff", border: "1px dashed rgba(201,168,76,0.3)" }}>
          <p style={{ fontFamily: "Cairo,sans-serif", color: "#1A6B47" }}>✅ لا توجد حوادث أو تحذيرات في آخر تقرير</p>
        </div>
      )}
