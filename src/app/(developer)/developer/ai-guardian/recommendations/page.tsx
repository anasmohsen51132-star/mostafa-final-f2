"use client";
// src/app/(developer)/developer/ai-guardian/recommendations/page.tsx
import Link from "next/link";
import { m as motion } from "framer-motion";
import { useQuery } from "@tanstack/react-query";
import { fetchWithAuth } from "@/hooks/useAuth";
import { RecommendationCard } from "@/components/developer/ai-guardian/GuardianCards";
...
export default function RecommendationsPage() {
  ...
  const recommendations = report?.reportJson.recommendations ?? [];
  const loading = isLoading || loadingReport;

  return (
    <div style={{ direction: "rtl" }}>
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
        <h1 style={{ fontFamily: "Amiri,serif", color: "#1A1208", fontSize: 32, marginBottom: 4 }}>📋 التوصيات</h1>
      ...
      {loading && <div className="space-y-3">{[...Array(3)].map((_, i) => <div key={i} className="skeleton rounded-2xl h-32" />)}</div>}
      ...
      {!loading && latestId && recommendations.length === 0 && (
        <div className="rounded-2xl p-10 text-center" style={{ background: "#fff", border: "1px dashed rgba(201,168,76,0.3)" }}>
          <p style={{ fontFamily: "Cairo,sans-serif", color: "#7A6E5A" }}>لا توجد توصيات في آخر تقرير</p>
        </div>
      )}

      {!loading && recommendations.length > 0 && (
        <div className="space-y-3">
          {recommendations.map((r, i) => <RecommendationCard key={i} recommendation={r} />)}
        </div>
      )}
    </div>
  );
